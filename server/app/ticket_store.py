...
class TicketStore:
    ...
    def _ensure_storage(self) -> None:
        with self._lock:
            ...
            if not self.file_path.exists():
                ...
                temp_file.write_text(json.dumps(initial_tickets, indent=2), encoding="utf-8")
    ...
    def _write(self, tickets: list[dict[str, Any]]) -> None:
        with self._lock:
            temp_file = self.file_path.with_suffix(".tmp")
            temp_file.write_text(json.dumps(tickets, indent=2, ensure_ascii=False), encoding="utf-8")
            temp_file.replace(self.file_path)
    ...
    def create_ticket(
        ...
        with self._lock:
            tickets = self._read()
            now_formatted = datetime.now(IST).strftime("%d %b %Y, %I:%M %p")
